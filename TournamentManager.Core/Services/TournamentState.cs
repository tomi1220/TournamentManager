...
namespace TournamentManager.Core.Services
{
    public class TournamentState
    {
        ...
        // 💾 【安全設計】Windowsの「ドキュメント」フォルダ内に専用フォルダを作成する設定
        // 💡 Linuxサーバー/Windowsローカルの両方で「プログラムの実行フォルダ直下」を指す安全な書き方
        ...
        private readonly string _filePath = Path.Combine(TargetFolder, "tournament_storage.json");
        ...
        {
            // 💡 起動時に保存先フォルダが存在するかチェックし、なければ自動作成
        ...
        private async Task loadMaster()
        {
            ...
            var jsonMasterTournamentNames =
                await httpClient.GetStringAsync("master/TournamentNamesMaster.json");
            MasterTournamentNames =
                System.Text.Json.JsonSerializer.Deserialize<List<TournamentNameMaster>>(
                    jsonMasterTournamentNames);
            var jsonMasterAreas =
                await httpClient.GetStringAsync("master/TournamentNamesMaster.json");
            MasterAreas =
                System.Text.Json.JsonSerializer.Deserialize<List<AreaMaster>>(
                    jsonMasterAreas);
            var jsonMasterTeams =
                await httpClient.GetStringAsync("master/TournamentNamesMaster.json");
            MasterTeams =
                System.Text.Json.JsonSerializer.Deserialize<List<TeamMaster>>(
                    jsonMasterTeams);
        }
        ...
        private void EnsureDirectoryExists()
        {
            try
            {
                ...
                {
                    DirectoryInfo directoryInfo = Directory.CreateDirectory(TargetFolder);
                    Console.WriteLine($"ドキュメント内に保存先フォルダを作成しました: {directoryInfo.FullName}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"フォルダの作成に失敗しました: {ex.Message}");
            }
        }
        ...
        public void LoadFromFile()
        {
            try
            {
                if (File.Exists(_filePath))
                {
                    ...
                    var data = JsonSerializer.Deserialize<TournamentSaveDataContainer>(json);
                    if (data != null)
                    {
                        SavedTournaments = data.SavedTournaments ?? new();
                        MasterSettings = data.MasterSettings ?? new();
                    }
                }
            }
            catch (Exception ex)
        ...
        public void SaveToFile()
        {
            try
            {
                ...
                var container = new TournamentSaveDataContainer
                {
                    SavedTournaments = this.SavedTournaments,
                    MasterSettings = this.MasterSettings
                };
        ...
        private class TournamentSaveDataContainer
        {
            public List<TournamentInfo> SavedTournaments { get; set; } = new();
            public AppMasterSettings MasterSettings { get; set; } = new();
        }
